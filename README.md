...
The same site as before, rebuilt in React (Vite), talking to the backend API in `augmented-competence-lab-backend.zip`. Tested end-to-end before delivery: built cleanly, served correctly, and a full login → create → edit → delete cycle was run against a live copy of the backend.
...
All six public sections stay mounted at once and are shown/hidden with a CSS class (same approach as the original static file) — this means if you're signed into Admin and switch to another tab, you stay signed in when you come back, since the Admin component never unmounts.
...
Point it at wherever the backend (see the other zip) is running.
